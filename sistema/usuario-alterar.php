...
if(isset($_POST["nomeUsuario"])){
    ...
    //alteração
    $alterar = "UPDATE tb_usuarios ";
    $alterar .= "SET ";
    $alterar .= "nomeUsuario = '{$nome}', ";	
    $alterar .= "senhaUsuario = '{$senha}' ";
    $alterar .= "WHERE idusuario = {$userID} ";
    ...
    //exclusao
    $exclusao = "DELETE FROM tb_usuarios ";
    $exclusao .= "WHERE idusuario = {$userID} ";
    $confirma_exclusao = mysqli_query($conecta, $exclusao);
    if(!$confirma_exclusao){
        die("Registro Não Pode Ser Excluído");
    }else{
        header("location:index.php");
    }
}
 //consulta tabela transportadora
 $tr = "SELECT * FROM tb_usuarios ";
 if(isset($_GET["codigo"])){
     $id = $_GET["codigo"];
     $tr .= "WHERE idusuario = {$id} ";
 }
 $con_transportadora = mysqli_query($conecta, $tr);
 if(!$con_transportadora){
     die("Erro na conexcao");
 }

 $info_usuario = mysqli_fetch_assoc($con_transportadora)
                ...
                <ul class="nav navbar-nav" id="aside-menu">
                    ...
                    <li class="menu-item-has-children dropdown">
                        ...
                        <ul class="sub-menu children dropdown-menu">
                            ...
                            <li><i class="menu-icon ti-themify-logo"></i><a href="usuario.php">Listar Usuários</a></li>
                        </ul>
                    </li>
        ...
        <div class="content">
            ...
            <div class="animated fadeIn">
                ...
                <div class="orders">
                    <div class="row">
                        <div class="col-xl-8">
                            <div class="card" id="box-top">
                                <div class="card-body" >
                                    <h4 class="box-title">Usuário <?php echo utf8_encode($info_usuario["nomeUsuario"]) ?></h4>
                                </div>
                    ...
                    <form method="post" action="usuario-alterar.php">
                        <div class="row">
                            <div class="col form-group">
                                <label for="inputUser">Nome</label>
                                <input type="text" class="form-control" name="nomeUsuario" id="inputUser" aria-describedby="emailHelp" value="<?php echo utf8_encode($info_usuario["nomeUsuario"]) ?>">                                                        
                            </div>
                            <div class="col form-group">
                                <label for="inputUser">E-mail</label>
                                <input type="email" class="form-control" name="emailUsuario" id="inputUser" aria-describedby="emailHelp" value="<?php echo $info_usuario["emailUsuario"]?>">                                                        
                            </div>
                            <div class="col form-group">
                                <label for="inputUser">Senha</label>
                                <input type="password" class="form-control" id="disabledInput" aria-describedby="emailHelp" value="<?php  echo $info_usuario["senhaUsuario"] ?>" disabled>                                                        
                            </div>                          
                        </div>
                        <div class="">
                            <div class="row">
                                <div class="col-md-2">
                                    <input type="hidden" name="idUsuario" value="<?php echo $info_usuario["idusuario"]?>">
                                    <input class="btn btn-primary" type="submit" id="btn-alterar-user" value="Confirmar Alteração"> 
                                </div>
                                <div class="col-md-2">
                                    <input type="hidden" name="idUsuario" value="<?php echo $info_usuario["idusuario"]?>">
                                    <input class="btn btn-danger" type="submit" value="Excluir Usuárioo">
                                </div>
                            </div>
                        </div>
                    </form>
